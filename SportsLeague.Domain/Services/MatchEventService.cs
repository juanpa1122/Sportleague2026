...
namespace SportsLeague.Domain.Services
{
    public class MatchEventService : IMatchEventService
    {
        ...
        public MatchEventService(IMatchRepository matchRepository, IMatchResultRepository matchResultRepository, IGoalRepository goalRepository, ICardRepository cardRepository, MatchValidationHelper validationHelper, ILogger<MatchEventService> logger)
        {
        ...
        public async Task<MatchResult> RegisterResultAsync(int matchId, MatchResult result)
        {
            ...
            _logger.LogInformation("Registering result for match {MatchId}: {Home}-{Away}", matchId, result.HomeGoals, result.AwayGoals);
        ...
        public async Task<Goal> RegisterGoalAsync(int matchId, Goal goal)
        {
            ...
            _logger.LogInformation("Registering goal: Match {MatchId}, Player {PlayerId}, Minute {Minute}", matchId, goal.PlayerId, goal.Minute);
        ...
        public async Task<Card> RegisterCardAsync(int matchId, Card card)
        {
            ...
            _logger.LogInformation("Registering {CardType} card: Match {MatchId}, Player {PlayerId}", card.Type, matchId, card.PlayerId);
